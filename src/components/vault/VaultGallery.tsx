...
export function VaultGallery({
  ...
  return (
    <div className="w-full min-h-screen pb-20">
      ...
      {/* Vault Hero Header */}
      <div className="relative w-full overflow-hidden rounded-3xl mb-8 glass-card border border-white/10 p-6 sm:p-10 shadow-2xl">
          ...
          <div className="space-y-2 max-w-2xl">
            <div className="flex items-center gap-2">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-cyan-500/20 text-cyan-300 border border-cyan-500/40">
                <CheckCircle2 className="w-3.5 h-3.5" /> PIN AUTHENTICATED
              </span>
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/30">
                <Radio className="w-3 h-3 animate-pulse text-emerald-400" /> Live Realtime Sync
              </span>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs text-slate-400 pt-2">
              ...
              <span>&bull;</span>
              <span>{formatBytes(mediaItems.filter(m => m.vault_id === vault.id).reduce((acc, cur) => acc + cur.size, 0))} total</span>
              <span>&bull;</span>
              <span>Downloads: {vault.download_enabled ? 'Enabled' : 'Restricted'}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            {vault.download_enabled && filteredItems.length > 0 && (
              ...
              >
                ...
                <span>Download All ({filteredItems.length})</span>
              </button>
            )}

            <button
              onClick={handleLock}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-semibold bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 border border-rose-500/30 transition-all hover:scale-105"
            >
      ...
      <div className="w-full flex flex-col gap-4 mb-8">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input
              type="text"
              placeholder="Search by title, tag, extension..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 rounded-2xl glass-input text-xs sm:text-sm font-medium"
            />
          </div>
          ...
          <div className="flex items-center gap-2 self-end sm:self-auto">
            ...
            <div className="flex items-center gap-1.5 px-3 py-2 rounded-xl glass-panel text-xs text-slate-300 border border-white/10">
              ...
              >
                ...
                <option value="size" className="bg-slate-900 text-white">Largest Size</option>
            ...
            <div className="flex items-center p-1 rounded-xl glass-panel border border-white/10">
              <button
                ...
                className={`p-1.5 rounded-lg transition-colors ${
                  gridColumns === 'grid' ? 'bg-cyan-500/20 text-cyan-300' : 'text-slate-400 hover:text-white'
                }`}
                title="Masonry Grid"
              >
              ...
              <button
                ...
                className={`p-1.5 rounded-lg transition-colors ${
                  gridColumns === 'compact' ? 'bg-cyan-500/20 text-cyan-300' : 'text-slate-400 hover:text-white'
                }`}
                title="Compact Grid"
              >
              ...
              .map((folder) => (
                ...
                >
                  <FolderIcon
                    ...
                    style={{ color: folder.color || '#06b6d4' }}
                  />
            ...
            >
              ...
              <div className="relative w-full aspect-square bg-slate-900 overflow-hidden flex items-center justify-center">
                ...
                {item.media_type === 'audio' && (
                  <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-purple-950/60 via-slate-900 to-indigo-950/60 p-4 text-center group-hover:scale-105 transition-transform">
                    <div className="w-14 h-14 rounded-full bg-purple-500/20 border border-purple-500/30 flex items-center justify-center text-purple-400 mb-2 shadow-glow-purple">
                      <Music className="w-7 h-7" />
                    </div>
                    <span className="text-[11px] font-medium text-slate-300 line-clamp-2 px-2">
                      {item.name}
                    </span>
                  </div>
                )}
                ...
                {item.media_type === 'document' && (
                  <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-emerald-950/50 via-slate-900 to-teal-950/50 p-4 text-center group-hover:scale-105 transition-transform">
                    <div className="w-14 h-14 rounded-2xl bg-emerald-500/20 border border-emerald-500/30 flex items-center justify-center text-emerald-400 mb-2 shadow-glow-emerald">
                ...
                {/* Media Type Badge */}
                <div className="absolute top-2.5 left-2.5 px-2 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider backdrop-blur-md bg-slate-950/70 text-slate-200 border border-white/10 flex items-center gap-1 shadow-md">
                  {item.media_type === 'image' && <ImageIcon className="w-3 h-3 text-cyan-400" />}
                  {item.media_type === 'video' && <Film className="w-3 h-3 text-blue-400" />}
                ...
                {/* Duration or Page Count Badge */}
                {item.metadata?.duration && (
                  <div className="absolute bottom-2.5 right-2.5 px-2 py-0.5 rounded-md text-[10px] font-mono bg-slate-950/80 text-cyan-300 backdrop-blur-sm border border-white/10">
                ...
                {(item.media_type === 'video' || item.media_type === 'audio') && (
                  <div className="absolute inset-0 flex items-center justify-center opacity-80 group-hover:opacity-100 transition-opacity">
                    <div className="w-12 h-12 rounded-full bg-cyan-500/90 text-slate-950 flex items-center justify-center shadow-glow-cyan transform group-hover:scale-110 transition-transform">
              ...
              <div className="p-3.5 flex flex-col justify-between flex-1">
                <h4 className="text-xs sm:text-sm font-semibold text-white truncate mb-1 group-hover:text-cyan-300 transition-colors">
                  {item.name}
                </h4>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {vault.download_enabled && (
                      <button
                        ...
                        className="p-1 rounded-lg bg-white/5 hover:bg-cyan-500/20 text-slate-400 hover:text-cyan-300 transition-colors"
